  ...
  for (i = 0; i < level.maxclients; i++)
    {
	  ...
	  && ((pvs)? trap_InPVS (ent->client->ps.origin, target->client->ps.origin) : IsVisible (ent, target, maxrange)))
        {
...
static void PointCamAtTarget (gentity_t *ent)
{
  ...
  if (ent == NULL
      ...
      || (ent->client->camera->flic_watching_the_wall
	  && !IsVisible (ent, ent->client->camera->flic_target, 0)))
    return;
  ...
  if (fabs(difference) > ent->client->camera->flic_angle_lag)
    {
	  ...
	  angles[1] += (difference <  20)? 
	    ent->client->camera->flic_angle_lag : ((1 + ( difference - 20) / 160) * ent->client->camera->flic_angle_lag);
        }
      else
        {
	  angles[1] -= (difference > -20)? 
...
static void FindCamPos (gentity_t *ent, float angle, vec3_t offset_position, vec3_t cam_pos)
{
  ...
  cam_pos[0] = ent->client->camera->flic_target->client->ps.origin[0] +
    (offset_position[0] * forward[0]);
        
  cam_pos[1] = ent->client->camera->flic_target->client->ps.origin[1] +
    (offset_position[1] * forward[1]);

  cam_pos[2] = ent->client->camera->flic_target->client->ps.origin[2] + 
    offset_position[2];
}
...
static void CameraFlicThink (gentity_t *ent)
{
  ...
  if (ent->client->camera->flic_target 
      && !ent->client->camera->flic_watching_the_dead 
      && InSolid (ent))
    {
  ...
  if (!ent->client->camera->flic_watching_the_dead 
      ...
      && ent->client->camera->flic_target->client->ps.pm_type == PM_DEAD)
    {
  ...
  else if (ent->client->camera->flic_watching_the_dead)
    {
	  ...
	  if (ent->client->camera->flic_target->client->ps.pm_type == PM_DEAD)
            {
	      VectorCopy(ent->client->camera->flic_target->client->ps.origin, ent->client->camera->flic_dead_origin);
            }
  ...
  // if we are done during a battle.
      ...
      if (IsVisible (ent, ent->client->camera->flic_target, 0))
	{
	  ...
	  VectorSubtract (ent->client->ps.origin, ent->client->camera->flic_target->client->ps.origin, diff);
...
static gentity_t *CameraSwingTarget (gentity_t *ent)
{
  ...
  if (target2nd != ent->client->camera->swing_secondry_target
      && target2nd != NULL
      && ent->client->camera->swing_msg_time <= level.time)
    {
      char *color;
...
static void CameraSwingAngle (gentity_t *ent)
{
  ...
  if (target == NULL) // return to swing_angle == 0
	...
	if (ent->client->camera->swing_angle > 0)
  ...
  if (target_distance > ent->client->camera->swing_distance)
    gamma = SWING_FOV_FACTOR * (acosf (ent->client->camera->swing_distance / target_distance)) * 180.0F / M_PI;
  else
    gamma = SWING_FOV_FACTOR * (90.0F - ((asinf (0.5 * target_distance / ent->client->camera->swing_distance)) * 180.0F / M_PI));
...
void camera_state_load (gclient_t *client)
{
  ...
  if (level.newSession == qtrue)
  ...
  else
    {
      ...
      cameras[clientNum].flic_target = NULL;
      if (target >= 0
	  && cameras[clientNum].mode == CAMERA_MODE_SWING)
