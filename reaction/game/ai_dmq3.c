...
float BotAggression(bot_state_t *bs) {
	...
	*/
	...
	if (bs->inventory[INVENTORY_MP5] > 0 &&
			bs->inventory[INVENTORY_MP5AMMO] > 10) return 90;
...
int BotHasPersistantPowerupAndWeapon(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_M4] > 0 &&
			bs->inventory[INVENTORY_M4AMMO] > 7) return qtrue;
	//if the bot can use the railgun
	if (bs->inventory[INVENTORY_SSG3000] > 0 &&
			bs->inventory[INVENTORY_SSG3000] > 5) return qtrue;
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	if (state->eType != ET_MISSILE || state->weapon != WP_GRENADE) return;
