...
tryagain:
	...
	if( pi->weaponModel == 0 ) {
		//Blaze: Changed WP_MACHINEGUN to WP_PISTOL - default weapon I think
...
void UI_DrawPlayer( float x, float y, float w, float h, playerInfo_t *pi, int time ) {
	...
	refEntity_t		head;
	refEntity_t		gun;
//Blaze: Used for spinning barrel, dont need for reaction
//	refEntity_t		barrel;
	refEntity_t		flash;
	vec3_t			origin;
	int				renderfx;
	...
	if ( pi->realWeapon == WP_MACHINEGUN || pi->realWeapon == WP_GAUNTLET || pi->realWeapon == WP_BFG ) {
		...
		//Blaze: Remove check for Gauntlet
...
void UI_PlayerInfo_SetModel( playerInfo_t *pi, const char *model ) {
	...
	//Blaze: Change base weapon from WP_MACHIEGUN to WP_PISTOL
	pi->weapon = WP_PISTOL;
...
//Blaze: no need to change stance
